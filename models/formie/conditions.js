...
export class FormieConditions {
  ...
  initFieldConditions() {
    this.$form.querySelectorAll('[data-field-conditions]').forEach(($field) => {
      ...
      conditionSettings.conditions.forEach((condition) => {
        ...
        const fieldName = condition.field.replace('{', '').replace('}', '')
        ...
        const $multiFields = this.$form.querySelectorAll(
          '[name="' + fieldName + '[]"]'
        )
        ...
        $targets.forEach(($target) => {
          ...
          $target.addEventListener(eventType, () =>
            $field.dispatchEvent(
              new CustomEvent('onFormieEvaluateConditions', { bubbles: true })
            )
          )
        })
      })
      ...
      $field.dispatchEvent(
        new CustomEvent('onFormieEvaluateConditions', {
          bubbles: true,
          detail: { init: true },
        })
      )
  ...
  evaluateConditions(e) {
    ...
    conditions.forEach((condition, i) => {
      ...
      $targets.forEach(($target) => {
        ...
        if ($target.getAttribute('data-fui-input-type') === 'agree') {
          ...
          result = this.testCondition(logic, value !== '0', $target.checked)

          results[resultKey].push(result)
        } else if (inputType === 'checkbox' || inputType === 'radio') {
          // Handle (multi) checkboxes and radio, which are a bit of a pain
          result =
            this.testCondition(logic, value, $target.value) && $target.checked
          ...
          Array.from($target.options).forEach(($option) => {
            result =
              this.testCondition(logic, value, $option.value) &&
              $option.selected
    ...
    if (isNested && !isInit) {
      const $parentField = $field.closest('.fui-type-group, .fui-type-repeater')
    ...
    $field.dispatchEvent(
      new CustomEvent('onAfterFormieEvaluateConditions', {
        bubbles: true,
        detail: { init: isInit },
      })
    )
