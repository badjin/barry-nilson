<template>
  <div>
    <PageHerosNews :module="hero" />
    <PageContentNews :module="body" />
...
<script>
import { trimStart, trimEnd, chunk, uniqBy } from 'lodash-es'
...
export default {
  ...
  async fetch() {
    try {
      ...
      await this.$store.dispatch('news/get', this.uri)

      if (this.hasOverride < this.moreArticlesCount) {
        this.doRefine()
      }
      this.$store.commit('entry/CHANGE_TYPE', 'news')
    } catch (e) {
      this.error('404', e)
    }
  ...
  methods: {
    async doRefine() {
      const res = await searchClient
        ...
        .search('', {
          hitsPerPage: 3,
          filters: `type:news AND NOT objectID:"${this.page.id}"  AND noIndex:false`,
        })
