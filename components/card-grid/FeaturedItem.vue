<template>
  <div class="flex justify-between">
    <div class="lg:w-6/12">
      <div class="mb-4">
        <span
          class="font-semibold text-xs font-display rounded-full bg-blue text-white px-3 py-1 leading-none"
        >
          Featured
        </span>
      </div>
    ...
    <div
      v-if="image"
      class="hidden lg:flex w-5/12 max-w-lg rounded overflow-hidden"
    >
      <div
...
export default {
  ...
  computed: {
    ...
    eventWhen() {
      const when = this.card.eventInformation?.length
        ? get(this.card.eventInformation, '[0].when')
        : ''
      return when
    },
    excerptTruncate() {
    ...
    image() {
      return get(this.card, 'image[0].src') || ''
    },
    categories() {
